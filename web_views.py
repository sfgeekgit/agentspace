...
def frame(body, active="overview", title="Workspace"):
    ...
    <div class="sidebar-bottom"><a class="nav-link {"selected" if active == "tools" else ""}" href="/tools">{icon('tools')}Advanced tools</a><a class="nav-link {"selected" if active == "help" else ""}" href="/help">{icon('help')}Getting started</a><div class="preview-label"><span class="status-dot"></span> GPT-6 preview <b>:7790</b></div></div></aside>
    <div class="workspace"><header class="topbar"><span>{esc(title)}</span><div class="topbar-right"><span class="local-tag">LOCAL WORKSPACE</span><a href="/help">Quick guide ↗</a></div></header><main id="content">{body}</main><footer>Agentspace <span>Explore. Run. Observe. Repeat.</span><span>Shared experiment data · independent UI</span></footer></div>
...
def scenario_card(s, snaps):
    ...
    return f'''<a class="scenario-card" data-search="{esc(s['name']+' '+s['description'])}" href="/scenarios/{u(s['name'])}"><div class="card-top"><span class="tile-icon">{icon('scenario')}</span><span class="tag">{esc(s.get('runtime') or 'unavailable')}</span></div><h3>{esc(s['name'])}</h3><p>{esc(s['description'])}</p><div class="card-bottom"><span>{count} agents · {'Game master' if s.get('has_gm') else 'Open interaction'}</span><span>↗</span></div><div class="card-footnote">{len(roots)} world roots · {'Available' if s['active'] else 'Inactive'}</div></a>'''
...
def overview():
    ...
    {flow()}<div class="stats-strip">{''.join(f'<a href="{href}"><strong>{n:02d}</strong><span>{label}</span>{icon(ico)}</a>' for n,label,href,ico in [(len(registry.list_scens(include_inactive=True)),'scenarios','/scenarios','scenario'),(len(roots),'world roots','/worlds','world'),(len(snaps)-len(roots),'saved snapshots','/worlds?view=snapshots','snap'),(len(envs),'environments','/environments','environment')])}</div>
...
def scenarios():
    ...
    snaps = db.list_snaps()
    warnings = ''.join(f'<div class="notice">Could not load {esc(p["name"])}: {esc(p["reason"])} {action("scen deactivate","Disable",{"scen_name":p["name"]}) if p["can_disable"] else ""}</div>' for p in problems)
    ...
    body += searchbar('Find a scenario…') + warnings + '<div class="cards three">' + ''.join(scenario_card(s,snaps) for s in scens) + '</div>'
...
def scenario_detail(name):
    ...
    body += f'<div class="fact-strip"><span><small>AGENTS</small>{s["min_agents"]}–{s["max_agents"]}</span><span><small>RUNTIME</small>{esc(s.get("runtime"))}</span><span><small>COORDINATION</small>{"Game master" if s.get("has_gm") else "Open interaction"}</span><span><small>WORLD ROOTS</small>{len(roots)}</span></div>'
    ...
    body += '<details class="section"><summary>Scenario maintenance</summary><div class="button-row">'+action('scen deactivate','Deactivate scenario',{'scen_name':name},note='This hides this scenario from new builds in this worktree. Existing worlds remain available.')+link('/tools?group=scen','Environment image tools','button secondary')+'</div></details>'
...
def tools_page(leaves,special,form_html):
    ...
    for group in dict.fromkeys(path[0] for path in leaves):
        ...
        body+=f'<h2 id="group-{group}">{title}</h2>'
    ...
    body+='</div><section class="section"><h2>Recent operations in this preview</h2><div id="runs"></div></section>'
