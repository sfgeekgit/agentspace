...
No runtime dependencies or Bellweather-specific branches. The model judges fiction;
...
def planning_context(s, b, action):
    return {"job": "PLAN", "move_number": s["turn"] + 1, "limit": s["params"]["max_turns"],
            ...
            "format": {"path": ["adjacent_node_id (up to 3, checked in order)"],
                       "effects": ["See operations below; all optional"],
                       "attributes": [{"name": "curiosity", "delta": 1, "reason": "specific evidence"}],
                       "talk": [{"npc": "present_after_moving", "topic": "one of their topics, or null"}],
                       "outcome": "Brief factual consequence; no speech. May be empty.", "end": None},
            "operations": [
...
def apply_plan(original, b, p):
    ...
    obj(p, ["path", "effects", "attributes", "talk", "outcome", "end"], "plan")
    s = copy.deepcopy(original)
    receipts, notices = [], []
    for dest in seq(p.get("path", []), 3, "path"):
    ...
    for r in s["work"]["replies"]:
        if r["visible"]:
            ...
            if r["say"]:
                pieces.append('“' + r["say"] + '”')
    if s["ended"]:
        pieces.append("The light softens over Bellweather. Your time here is complete." if s["ended"] == "turn_cap"
