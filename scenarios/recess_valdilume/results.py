...
python3 scenarios/recess_valdilume/results.py ENV
python3 scenarios/recess_valdilume/results.py NAME --state state.json --out /tmp/results
...
def main():
    ...
    with (args.out / ".runs.lock").open("a") as lock:
        ...
        rows.append({"run_name": args.name, "scen": "recess_valdilume", "turns": state["turn"],
