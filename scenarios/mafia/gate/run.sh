...
# shared gate harness (runtime_pi/dispatch_gate/setup_world.sh + scripted dummy).
...
# runtime base) — so a dispatcher that imports env-provided libraries gates against
...
gate() { # mode hard_bool
    local wj="{\"has_dispatch\": true, \"params\": {\"discussion_passes\": 1, \"hard_enforcement\": $2}}"
    docker run --rm --network none --user 0:0 -e MODE="$1" \
        ...
        bash -c "bash /repo/runtime_pi/dispatch_gate/setup_world.sh \
                     scenarios/mafia/dispatch/main.py '$wj' scenarios/mafia/gate/moves '$SECRETS' \
