...
rest villagers. Deterministic dispatcher (design:
...
kill, detective investigate — result arrives in their next dispatcher message —
...
- **Hidden info at build**: `logic.fill_briefing` names each mafia's partners
  in their briefing; `logic.dispatch_secrets` bakes the answer key to
  `/dispatch/secrets.json` (agents can't read it; also in the build audit record).
- **`hard_enforcement` build param (decision 12)** — one scen, two world
  physics. Hard: live `dispatch_policy` allowlists (day = public posts only /
  night = mafia↔mafia PMs only; public posting is a policy pair
  `[sender, "public"]`). Soft: identical rules as briefing NORMS only; the
  dispatcher referees overnight violations from `api.activity()` metadata and
  announces them. Playtesting both is the experiment.
- Elimination via `dispatch_remove` (no wakes, no sends), pull-only public board,
  every activation caused by a logged dispatcher wake or PM delivery.
...
shared harness (`runtime_pi/dispatch_gate/setup_world.sh` + scripted dummy).

Watch a run: `env logs <name> -f` (gateway audit), the public board for the
game as spectators see it, `/dispatch/state.json` for the true state.
