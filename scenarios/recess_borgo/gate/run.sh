#!/bin/bash
# recess_borgo scen gate: a scripted 7-turn game against the real engine in a
...
s = registry.load_scen('recess_borgo')
...
docker run --rm --network none --user 0:0 -v /opt/agentspace-ctl:/repo:ro "$IMG" \
    bash -c "bash /repo/runtime_pi/dispatch_gate/setup_world.sh \
                 scenarios/recess_borgo/dispatch/main.py '$WJ' scenarios/recess_borgo/gate/moves '$SECRETS' \
             && cp -r /repo/scenarios/recess_borgo/dispatch/. /dispatch/code/ && chown -R dispatch /dispatch \
             && python3 /repo/scenarios/recess_borgo/gate/gate.py"
echo "RECESS_BORGO GATE: ALL PASS"
