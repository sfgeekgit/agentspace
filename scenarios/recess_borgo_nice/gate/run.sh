#!/bin/bash
# recess_borgo_nice scen gate: a scripted 7-turn game against the real engine in a
...
s = registry.load_scen('recess_borgo_nice')
...
docker run --rm --network none --user 0:0 -v /opt/agentspace-ctl:/repo:ro "$IMG" \
    bash -c "bash /repo/runtime_pi/dispatch_gate/setup_world.sh \
                 scenarios/recess_borgo_nice/dispatch/main.py '$WJ' scenarios/recess_borgo_nice/gate/moves '$SECRETS' \
             && cp -r /repo/scenarios/recess_borgo_nice/dispatch/. /dispatch/code/ && chown -R dispatch /dispatch \
             && python3 /repo/scenarios/recess_borgo_nice/gate/gate.py"
echo "RECESS_BORGO_NICE GATE: ALL PASS"
