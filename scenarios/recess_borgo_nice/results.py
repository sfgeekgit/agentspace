#!/usr/bin/env python3
"""End-of-run results for a recess_borgo_nice env (run on the HOST):

    python3 scenarios/recess_borgo_nice/results.py <env>     # -> $AGENTSPACE_RESULTS_DIR/<env>/
...
def main():
    ...
    old = [l for l in runs.read_text().splitlines() if l.strip() and json.loads(l).get("run_name") != env] \
        if runs.exists() else []
    line = {"run_name": env, "scen": "recess_borgo_nice", "status": state["ended"] or "capped",
