"""Ticket seeds — the shift's raw material. Read at build by logic.py only; each
seed is baked into one customer's private ROLE.md and nowhere else.
...
Keep at least scenario.toml's max_agents - 1 seeds (one customer per ticket).
"""
...
TICKETS = [
    ...
    {"name": "Leo",
     "situation": "Shares holiday albums with his grandparents, who have no account.",
     "problem": "The album link he sent his grandparents last month now says "
                "'this link has expired'. He never set anything to expire.",
     "cause": "The link was created with the default seven-day expiry in the "
              "share dialog. A new link with no expiry has to be made and sent."},

    {"name": "Hana",
     "situation": "iPhone user, five years of photos, backs up from the phone only.",
     "problem": "Everything is backed up, but online the photos look soft and "
                "blocky when zoomed in and most videos are only a few seconds long.",
     "cause": "The phone's iCloud 'Optimise Storage' setting keeps only small "
              "previews on the device, and those are what the app uploaded. The "
              "originals have to be downloaded to the phone and re-uploaded."},
]
